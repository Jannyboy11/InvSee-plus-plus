...
class EnderNmsContainer extends AbstractContainerMenu {
	...
	public ItemStack quickMoveStack(Player player, int rawIndex) {
		...
		Slot slot = super.getSlot(rawIndex);																		// Mohist compat: use 'super' instead of 'this'
		...
		if (slot != null && slot.hasItem()) {
			...
			if (rawIndex < topRows * 9) {
				...
				if (!super.moveItemStackTo(clickedSlotItem, topRows * 9, super.slots.size(), true)) {		// Mohist compat: use 'super' instead of 'this' (twice)
					return ItemStack.EMPTY;
				}
			} else {
				//clicked in the bottom inventory
				if (!super.moveItemStackTo(clickedSlotItem, 0, topRows * 9, false)) {					// Mohist compat: user 'super' instead of 'this'
					return ItemStack.EMPTY;
				}
			}
