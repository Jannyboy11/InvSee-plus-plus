...
class MainNmsContainer extends AbstractContainerMenu {
	...
	public ItemStack quickMoveStack(Player entityHuman, int rawIndex) {
		...
		final Slot slot = super.getSlot(rawIndex);																	// Mohist compat: call super.getSlot instead of this.getSlot
		final int topRows = 6;
		...
		if (slot != null && slot.hasItem()) {
			...
			if (rawIndex < topRows * 9) {
				...
				if (!super.moveItemStackTo(clickedSlotItem, topRows * 9, super.slots.size(), true)) {		// Mohist compat: call super (twice)
					return ItemStack.EMPTY;
				}
			} else {
				//clicked in the bottom inventory
				if (!super.moveItemStackTo(clickedSlotItem, 0, topRows * 9, false)) {					// Mohist compat: call super
					return ItemStack.EMPTY;
				}
			}
