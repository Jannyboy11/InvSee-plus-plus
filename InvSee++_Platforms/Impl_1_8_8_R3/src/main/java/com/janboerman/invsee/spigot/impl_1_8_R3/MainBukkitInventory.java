...
public class MainBukkitInventory extends CraftInventory implements MainInventory<MainNmsInventory, MainBukkitInventory> {
    ...
    public ItemStack[] getStorageContents() {
        return Arrays.stream(getInventory().storageContents).map(CraftItemStack::asCraftMirror).toArray(ItemStack[]::new);
    }
    ...
    public void setStorageContents(ItemStack[] storageContents) {
        ...
        int storageContentsSize = getInventory().storageContents.length;
        ...
        for (int i = 0; i < storageContentsSize; i++) {
            getInventory().storageContents[i] = CraftItemStack.asNMSCopy(storageContents[i]);
        }
    }

    @Override
    public ItemStack[] getArmourContents() {
        return Arrays.stream(getInventory().armourContents).map(CraftItemStack::asCraftMirror).toArray(ItemStack[]::new);
    }
    ...
    public void setArmourContents(ItemStack[] armourContents) {
        ...
        int armourContententsSize = getInventory().armourContents.length;
        ...
        for (int i = 0; i < armourContententsSize; i++) {
            getInventory().armourContents[i] = CraftItemStack.asNMSCopy(armourContents[i]);
        }
    }
    ...
    public void setPersonalContents(ItemStack[] craftingContents) {
        ...
        if (nmsCraftingItems != null) {
            ...
            for (int i = 0; i < craftingContentsSize; i++) {
                nmsCraftingItems[i] = CraftItemStack.asNMSCopy(craftingContents[i]);
            }
        }
    }
