...
class MainBukkitInventory extends CraftInventory implements MainInventory<MainNmsInventory, MainBukkitInventory> {
	...
	public ItemStack[] getOffHandContents() {
		...
		return new ItemStack[] {
				CraftItemStack.asBukkitMirror(inv.getItem(40)),
				CraftItemStack.asBukkitMirror(inv.getItem(41)),
				CraftItemStack.asBukkitMirror(inv.getItem(42))
 		};
	}
	...
	public ItemStack getCursorContents() {
		var onCursor = getInventory().onCursor;
		if (onCursor != null) {
			return CraftItemStack.asBukkitMirror(onCursor.get());
		} else {
			return null;
		}
	}
	...
	public ItemStack[] getPersonalContents() {
		...
		if (nmsCraftingItems != null) {
			return nmsCraftingItems.stream().map(CraftItemStack::asBukkitMirror).toArray(ItemStack[]::new);
		} else {
	...
	public int firstPartial(ItemStack item) {
		...
		item = CraftItemStack.getCraftStack(item);
		for (int slot = 0; slot < getSize(); slot++) {
